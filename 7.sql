...
SELECT name, description FROM v$BGPROCESS ORDER BY 1;--V$Bgprocess. Оно содержит, список возможных фоновых процессов, причем в представление включен дополнительный столбец PADDR в котором выводится шестнадцатеричный адрес выполняемого фонового процесса. Когда процесс не запущен столбец PADDR имеет значение 0.
...
--6.	Определите сервисы (точки подключен ия экземпляра).
